...
int main(int argc, char *argv[])
{
  ...
  bool guardArgs = false; // bool guard for correct number of arguments
  bool guardN = false;    // bool guard for correct range of input argument 'n'
  bool guardFile = false; // bool guard for correct filepath
  ...
  FILE *fileArg;
  fileArg = fopen(argv[2], "r+"); // 'r+', read and write
  if (fileArg != NULL)            // if the file can be opened
  {
    guardFile = true;
  }

  if (guardArgs && guardN && guardFile) // if all the guards are true
  {
    // everything is set, program continues
    pid_t parent_PID = getpid(); // retrieving parent PID before fork() so as to be executed first and only once by the original process (parent process)
    for (int i = 0; i < n; i++)  // creating n childs
    {
      pid_t child_PID;              // variable to store child ProcessID after fork()
      switch ((child_PID = fork())) // using switch statement instead of if-else for code elegance
      {
      case -1: // fork() returns -1 if there was an error
        printf(ANSI_COLOR_RED "ERROR: fork() error\n" ANSI_COLOR_RESET);
        returnCode = 13; // return code set to 13, custom value to indicate fork error, incrementing value from the previous error codes
        break;
      ...
      case 0: // fork() returns 0 to the child
        ...
        sigset_t blockedList, old;        // defining signal list data structures
        sigfillset(&blockedList);         // adding ALL signals to the list
        sigdelset(&blockedList, SIGUSR1); // removing specific signals from the list
        sigdelset(&blockedList, SIGUSR2);
        sigdelset(&blockedList, SIGTSTP);
        ...
        sigprocmask(SIG_SETMASK, &blockedList, &old); // we can't directly modify the signal mask but we can do it by using sigprocmask()
        ...
        struct_child.sa_mask = blockedList;
      ...
      /* THIS SECTION IS EQUAL AS SWITCH, BUT WITH IF STATEMENT
  ...
  } // if some requirements have not been met
  else if (!guardArgs) // if wrong number or arguments
  {
  ...
  else if (!guardN) // if 'n' is not between 1 and 20 included
  {
  ...
  else if (!guardFile) // if the file couldn't be opened
  {
    returnCode = 12; // return code set to 12, as specified in case <file> is not readable
    printf(ANSI_COLOR_RED "ERROR: couldn't read <file> parameter\n" ANSI_COLOR_RESET);
    printf("USAGE: ./a.out <n> <file>\n\ti.e. ./a.out 3 tmp/scriviqui.txt\n");
  }
  return returnCode;
}
